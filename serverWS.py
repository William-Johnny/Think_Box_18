...
class Message:
    ...
    def __init__(self, src: str, dest: str, uid: str, optionId: int) -> None:
        self.src = src
        self.dest = dest
        self.uid = uid
    ...
    def from_dict(obj: Any) -> 'Message':
        ...
        optionId = from_str(obj.get("optionId"))
        return Message(src, dest, uid, optionId)
    ...
    def to_dict(self) -> dict:
        ...
        result["optionId"] = from_str(self.optionId)
        return result
...
class WSServer:
    ...
    def message_received(self,client, server, message):
        ...
        if self.client_dict.get(client['id']) is None:
            ...
            print(f"Message reçu de {self.client_dict[client['id']]} pour {obj.dest} : {obj.uid} {obj.optionId}")
            ...
            if self.client_exists(obj.dest) == False:
                ...
                print(obj.uid)
                if obj.uid != "Unknown uid":
                    esp_client = self.client_obj_dict[self.get_client_from_name("ESP32")]
                    self.server.send_message(esp_client, json.dumps({"rfid": obj.uid}))
...
wsServer = WSServer(8080, "192.168.10.149")
